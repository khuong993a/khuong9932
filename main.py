...
def read_data_file(filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            accounts.append({'email': email, 'token': token, 'proxy': proxy})
    return accounts

def get_total_points(headers, proxies):
    try:
...
def make_request(headers, email, proxies):
    keepalive_payload = {
        "username": email,
...
def main():
    ...
    while True:
        ...
        for account in accounts:
            ...
            points = get_total_points(headers, proxies)
            total_accumulated_points += points

            success = make_request(headers, email, proxies)
            if success:
